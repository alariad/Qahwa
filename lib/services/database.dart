...
class DatabaseService {
  ...
  final CollectionReference quserCollection = Firestore.instance.collection('Users');
  ...
  Future<void> updateUserProgress(int progress, String name) async {
    return await quserCollection.document(uid).setData({
      'progress': progress,
      'name' : name,
    });
  }
*/
 ...
 /* Stream<QuerySnapshot> get brews {
  ...
  // get users stream
  Stream<List<Quser>> get users {
    return quserCollection.snapshots().map(_quserListFromSnapshot);
  }

  // brew list from snapshot
  List<Quser> _quserListFromSnapshot(QuerySnapshot snapshot) {
    return snapshot.documents.map((doc){
      //print(doc.data);
      return Quser(
          progress: doc.data['prgress'] ?? 0
      );
    }).toList();
  }
*/
